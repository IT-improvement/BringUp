    ...
    `;
        ...
        function fetchBlogList() {
            ...
            fetch("/portfolio/blog/list", {
                ...
                .then(data => {
                    if (data.code === "SU" && Array.isArray(data.list)) {
                        blogData = data.list; // 블로그 데이터 저장
                        displayBlogModalList(blogData); // 모달에 데이터 표시
                    } else {
                        alert("블로그 데이터를 가져오는 데 실패했습니다.");
                    }
                })
                .catch(error => {
        ...
        // 블로그 리스트를 모달에 표시
        function displayBlogModalList(blogs) {
            const blogList = document.getElementById("blogList");
            blogList.innerHTML = ""; // 기존 리스트 초기화

            if (!blogs || blogs.length === 0) {
                blogList.innerHTML = "<p class='text-muted'>등록된 블로그가 없습니다.</p>";
                return;
            }
            ...
            blogs.forEach(blog => {
                ...
                listItem.innerHTML = `
            <div>
                <h6>${'${blog.title || "제목 없음"}'}</h6>
                <p class="mb-0 text-muted">${'${blog.url || "URL 없음"}'}</p>
            </div>
            <button class="btn btn-outline-primary btn-sm" onclick="addBlogToResume(${'${blog.id}'})">추가</button>
        `;
        ...
        // 블로그를 이력서 카드에 추가
        function addBlogToResume(blogId) {
            ...
            // 이미 추가된 블로그인지 확인
            if (document.getElementById(`blog-card-${'${blogId}'}`)) {
                alert("이미 추가된 블로그입니다.");
                return;
            }
            ...
            const card = document.createElement("div");
            card.className = "card mb-3";
            card.id = `blog-card-${'${blogId}'}`;
            card.innerHTML = `
        <div class="card-body">
            <h5>${'${blog.title || "제목 없음"}'}</h5>
            <p>${'${blog.url || "URL 없음"}'}</p>
            <button class="btn btn-sm btn-danger mt-2" onclick="removeBlog(${'${blogId}'})">삭제</button>
        </div>
    `;
        ...
        // 페이지 로드 시 블로그 데이터를 가져오기
        document.addEventListener("DOMContentLoaded", () => {
            const blogAddButton = document.querySelector(".experience button[data-bs-toggle='modal']");
            if (blogAddButton) {
                blogAddButton.addEventListener("click", fetchBlogList);
            } else {
                console.error("블로그 추가 버튼을 찾을 수 없습니다.");
            }
        });
