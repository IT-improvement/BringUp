...
public class RecruitmentController {
    ...
    public ResponseEntity<BfResponse<?>> registerRecruitment(@RequestHeader("Authorization") String token,
                                                             @RequestBody RecruitmentRequestDto requestDto) {
        recruitmentService.createRecruitment(token, requestDto);
    ...
    public ResponseEntity<BfResponse<?>> updateRecruitment(@RequestHeader("Authorization") String token,
                                                           @PathVariable Integer recruitmentId,
                                                           @RequestBody RecruitmentRequestDto requestDto) {
        recruitmentService.updateRecruitment(token, recruitmentId, requestDto);
    ...
    public ResponseEntity<BfResponse<?>> deleteRecruitment(@RequestHeader("Authorization") String token,
                                                           @PathVariable Integer recruitmentId,
                                                           @RequestBody String reason) {
        recruitmentService.deleteRecruitment(token, recruitmentId, reason);
        return ResponseEntity.ok(new BfResponse<>(SUCCESS, "Recruitment deletion request submitted successfully"));
    }

    @GetMapping("/list")
    public ResponseEntity<BfResponse<List<RecruitmentResponseDto>>> listRecruitments(@RequestHeader("Authorization") String token,
                                                                                     @RequestParam Long companyId) {
        List<RecruitmentResponseDto> recruitments = recruitmentService.getRecruitments(companyId);
